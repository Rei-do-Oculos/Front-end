...
import { Card, Button, Input, SingleSelect, AccessDeniedCard } from '../../components/Common';
import { useParams, useNavigate } from 'react-router-dom';
import { useAdminExpenses } from '../../services/hooks/useAdminExpenses';
...
export const AdminExpenseForm: React.FC = () => {
  const { id } = useParams();
  ...
  const { getExpense, createExpense, updateExpense } = useAdminExpenses({ autoFetch: false });
  ...
  const storeOptions = storesList.map((s) => ({
    value: String(s.id),
    label: s.name || s.unity || `Loja ${s.id}`,
  }));

  useEffect(() => {
    fetchStores(1, { per_page: 200 });
  }, [fetchStores]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!isEditMode && !formData.store_id) {
      setErrors({ store_id: 'Selecione a loja' });
      showError('Selecione a loja');
      return;
    }
    if (!name) {
    ...
    try {
      ...
      if (!isEditMode) {
        payload.store_id = parseInt(formData.store_id, 10);
      ...
      if (isEditMode && id) {
        ...
        showSuccess('Despesa administrativa atualizada com sucesso!');
      } else {
        await createExpense(payload);
        showSuccess('Despesa administrativa criada com sucesso!');
      }
      navigate('/finance/admin-expenses');
    } catch (err: any) {
  ...
  const canCreate = hasPermission('expenses-admin.create');
  const canUpdate = hasPermission('expenses-admin.update');
  const canRead = hasPermission('expenses-admin.read');
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-3xl font-black text-slate-950 tracking-tight">
              {isEditMode ? 'Editar despesa administrativa' : 'Nova despesa administrativa'}
            </h1>
            <p className="text-gray-500 font-medium mt-1">
              Registro exclusivo da gestão; separado das despesas operacionais da equipe.
            </p>
          </div>
        </div>
      </div>
        ...
        <Card className="p-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <SingleSelect
              ...
              placeholder={storeOptions.length ? 'Selecione a loja' : 'Carregando lojas...'}
            />
            {errors.store_id && <p className="text-xs text-red-500 md:col-span-2">{errors.store_id}</p>}
            <Input
              label="Nome da despesa *"
              placeholder="Ex: Aluguel matriz, contador, sistema"
              value={formData.name}
          ...
          <div className="flex gap-4 mt-8 pt-6 border-t border-slate-100">
            <Button type="button" variant="outline" onClick={() => navigate('/finance/admin-expenses')} disabled={saving}>
            ...
            <Button type="submit" disabled={saving}>
              {saving ? (
                ...
                <><Save size={18} /> {isEditMode ? 'Atualizar' : 'Criar'}</>
              )}
            </Button>
          </div>
        </Card>
